...
def evaluate_observation(mlp, args):
    ...
    desdfs = {}
    semantic_desdfs = {}
    for scene in tqdm.tqdm(train_set.scene_names):
        # Load desdf
    ...
    acc_record = []
    acc_orn_record = []
    for data_idx in tqdm.tqdm(range(len(train_set))):
        data = train_set[data_idx]
        # Get the scene name according to the data_idx
        scene_idx = np.sum(data_idx >= np.array(train_set.scene_start_idx)) - 1
        scene = train_set.scene_names[scene_idx]

        # Get idx within scene
        idx_within_scene = data_idx - train_set.scene_start_idx[scene_idx]
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for data_idx in tqdm.tqdm(range(len(train_set))):
            ...
            gt_y, gt_x = int(gt_pose_desdf[1]), int(gt_pose_desdf[0])
            gt_orn = int((gt_pose_desdf[2] / (2 * np.pi) * desdf["desdf"].shape[2]))
            gt_prob = prob_vol_pred[gt_y, gt_x, gt_orn]
            all_probs = prob_vol_pred.flatten()

            all_probs = torch.tensor(all_probs, dtype=torch.float32, requires_grad=False)
            sorted_probs, _ = torch.sort(all_probs, descending=True)
            where_result = torch.where(sorted_probs == gt_prob)[0]
            if where_result.numel() > 0:
                rank = where_result[0].item()  
            else:
                rank = 0

            rank_tensor = torch.tensor(rank, dtype=torch.float32, requires_grad=True)
            len_all_probs = torch.tensor(len(all_probs), dtype=torch.float32, requires_grad=False)
            loss = rank_tensor / len_all_probs
    ...
    torch.save(mlp.state_dict(), mlp_checkpoint_path)
    print(f"MLP checkpoint saved to {mlp_checkpoint_path}")
